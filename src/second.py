...
def generate_recommendations(prompt, bad_answer, n=5):
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"Ты ассистент, который улучшает промпты, ты можешь писать ТОЛЬКО улучшенный промпт и больше ничего. Предложи улучшенный промпт на основе запроса, который указывает пользователь (максимум 500 токенов).\nВот руководство, которым ты можешь пользоваться для улучшения промптов\n\n###НАЧАЛО РУКОВОДСТВА###\n{metaprompt}"
            },
            {
                "role": "user", 
